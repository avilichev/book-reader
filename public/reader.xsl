...
<xsl:stylesheet
  ...
  <xsl:template match="/*">
    <div class="fb2-book">
      ...
      <xsl:for-each select="fb:description/fb:title-info/fb:annotation">
        <section class="fb2-book__annotation">
      ...
      <xsl:for-each select="fb:body">
        <xsl:if test="position() != 1">
          <div class="fb2-book-separator"></div>
        </xsl:if>

        <div class="fb2-book-wrapper">
          <xsl:apply-templates/>
        </div>
      </xsl:for-each>
    </div>
  ...
  <xsl:template match="fb:section">
    <section>
      <xsl:attribute name="class">
        <xsl:text>fb2-book-section</xsl:text>
  ...
  <xsl:template match="fb:epigraph/fb:text-author">
    <blockquote class="fb2-book-author">
      <i>
        <xsl:apply-templates/>
      </i>
    </blockquote>
    ...
    <div>
      <xsl:choose>
        <xsl:when test="ancestor::fb:coverpage">
          <xsl:attribute name="class">
            <xsl:value-of select="'fb2-book__cover'"/>
          </xsl:attribute>
        </xsl:when>
        <xsl:otherwise>
          <xsl:attribute name="class">
            <xsl:text>image fb2-book-image</xsl:text>
          </xsl:attribute>
        </xsl:otherwise>
      </xsl:choose>
      <div class="fb2-book-image__wrapper">
        <img>
          <xsl:choose>
            <xsl:when test="starts-with(@xlink:href,'#')">
      ...
      <xsl:if test="following-sibling::node()[1]/fb:emphasis">
        <div class="fb2-book-image__annotation">
